...
function solicitacoesController() {
  ...
  async function processarSolicitacao(req, res) {
    const { id } = req.params; 
    const { decisao, usuario_id_terceiro } = req.body; 
    const { role } = req.user;
    
    if (role !== 'admin') {
      return res.status(403).json({ error: 'Apenas administradores podem processar solicitações.' });
    }
  
    try {
      const resultado = await repoSolicitacoes.processarSolicitacao(id, decisao, usuario_id_terceiro);
